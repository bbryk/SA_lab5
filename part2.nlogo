...
to make-turtles
  ...
  create-turtles num-joss [set strategy "joss" set color yellow ]
  create-turtles num-anon [set strategy "anon" set color brown ]
  create-turtles num-custom [set strategy "custom" set color orange ]
end
...
to update-history
  ...
  if strategy = "anon" [ anon-history-update ]
  if strategy = "custom" [ custom-history-update ]
end
...
to anon
  ...
  set num-anon-games num-anon-games + 1
  let c random-float 1.0
  ifelse (c > 0.3 and c < 0.7) [
      set defect-now? false
    ] [
      set defect-now? true
    ]
...
to custom
  set num-custom-games num-custom-games + 1
set partner-defected? item ([who ] of partner) partner-history

  ifelse (partner-defected?)
    [set defect-now? true]
  [ ifelse ( [who - 3]of partner > 0 ) [
    set partner-defected1? item ([who - 3] of partner) partner-history
    set partner-defected2? item ([who - 2] of partner) partner-history
    ifelse (partner-defected1? and partner-defected2? )
    [set defect-now? true][
      set defect-now? false
    ]
...
to do-scoring
  ...
  set anon-score  (calc-score "anon" num-anon)
  set custom-score  (calc-score "custom" num-custom)
end
...
"joss" 1.0 0 -1184463 true "" "if num-joss-games > 0 [ plot joss-score / (num-joss-games) ]"
"anon" 1.0 0 -10402772 true "" "if num-anon-games > 0 [ plot anon-score / (num-anon-games) ]"
"custom" 1.0 0 -955883 true "" "if num-custom-games > 0 [ plot custom-score / (num-custom-games) ]"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <metric>unknown-score / (num-unknown-games)</metric>
    <metric>joss-score / (num-joss-games)</metric>
    <metric>anon-score / (num-anon-games)</metric>
    <enumeratedValueSet variable="n-unforgiving">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-joss">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="n-anon">
      <value value="50"/>
    </enumeratedValueSet>
